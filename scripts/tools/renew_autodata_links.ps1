# renew_autodata_links.ps1 - Generate presigned URLs for all files in S:\Autodata
...
$JSON_OUT  = "$REPO_DIR\autodata_links.json"
$EXPIRES   = "2h"
$e         = [char]27
if (-not (Test-Path $RCLONE)) {
    [System.Windows.Forms.MessageBox]::Show("rclone nao encontrado:`n$RCLONE","Renew - Erro",[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Error) | Out-Null
    exit 1
}
if (-not (Test-Path $SCAN_DIR)) {
    [System.Windows.Forms.MessageBox]::Show("Pasta nao encontrada: $SCAN_DIR`n`nVerifica se o drive S:\ esta mapeado.","Renew - Erro",[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Error) | Out-Null
    exit 1
}
$allFiles = Get-ChildItem $SCAN_DIR -Recurse -File -ErrorAction SilentlyContinue | Sort-Object FullName
$Host.UI.RawUI.WindowTitle = "M-Auto - Renew Autodata Links"
Clear-Host
Write-Host "  ${e}[38;2;231;76;60m+--------------------------------------------------+${e}[0m"
Write-Host "  ${e}[38;2;231;76;60m|${e}[0m  ${e}[1;97mM-Auto${e}[0m  ${e}[38;2;231;76;60mRenew Autodata Links${e}[0m"
Write-Host "  ${e}[38;2;231;76;60m+--------------------------------------------------+${e}[0m"
...
$files = foreach ($f in $allFiles) {
    ...
    $label   = if ($f.DirectoryName -eq $SCAN_DIR) { $f.Name } else { "$($f.DirectoryName.Substring($SCAN_DIR.Length).TrimStart('\'))\$($f.Name)" }
    [PSCustomObject]@{ label = $label; dest = $relPath; r2 = "$R2_PREFIX/$relPath" }
}
...
$jobs = foreach ($f in $files) {
    ...
    [void]$ps.AddScript({ param($rclone, $bucket, $r2path, $expires); $remote = "$bucket/$r2path"; $url = & $rclone link $remote --expire $expires 2>&1; return @{ ok = ($LASTEXITCODE -eq 0); url = "$url".Trim() } }).AddParameters(@{ rclone = $RCLONE; bucket = $BUCKET; r2path = $f.r2; expires = $EXPIRES })
...
try {
    $null = git add autodata_links.json 2>&1
    git commit -m "renew: autodata_links.json $ts ($($filesList.Count) ficheiros)" 2>&1 | ForEach-Object { Write-Host "  ${e}[38;2;80;100;140m$_${e}[0m" }
    git push 2>&1 | ForEach-Object { Write-Host "  ${e}[38;2;80;100;140m$_${e}[0m" }
    Write-Host "  ${e}[38;2;34;197;94m[OK]${e}[0m  Publicado em m-auto.online/autodata_links.json"
} catch {
