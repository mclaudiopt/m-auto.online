...
function Invoke-Download {
    ...
    while (-not $process.HasExited) {
        ...
        if (Test-Path $dest) {
            ...
            Write-Host -NoNewline "`r  ${e}[38;2;255;204;0m$spinner${e}[0m $dlMB MB  $spdStr  ${e}[38;2;148;163;184m[CN:16] [$Idx/$Total]${e}[0m  "
        }
    }
...
while ($retry -lt $maxRetries) {
    ...
    if ($links) {
        ...
        Write-Host "    ${e}[38;2;255;204;0m[A]${e}[0m Transferir todos os ficheiros em falta"
        Write-Host "    ${e}[38;2;255;204;0m[1-$($links.Count)]${e}[0m Transferir ficheiro especifico"
        ...
        if ($choice -eq "C" -or $choice -eq "c") {
            $cleanScript = Join-Path $PSScriptRoot "renault_clean.ps1"
        ...
        foreach ($idx in $toDownload) {
            ...
            Write-Host "  ${e}[38;2;255;204;0m-- $($f.name) ($($ok+$fail+1)/$($toDownload.Count)) --${e}[0m"
