...
$VERSION  = "1.1 [2026-04-02 16:11:09]"
...
function Show-Tools {
    while ($true) {
        ...
        Write-Opt 4  "Instalar Free Download Manager"
        Write-Host ""
        Write-Host "  ${e}[38;2;100;149;237m[Sistema]${e}[0m"
        Write-Opt 5  "DControl - gerir Windows Defender"
        Write-Opt 6  "Criar ponto de restauro do sistema"
        Write-Opt 7  "Configurar Home Page (m-auto.online)"
        Write-Opt 8  "Listar interfaces J2534"          "PassThru instalados"
        Write-Opt 9  "Speedtest"                        "Teste de velocidade"
        Write-Opt 10 "Activar Windows"
        Write-Host ""
        Write-Host "  ${e}[38;2;100;149;237m[Customizacao]${e}[0m"
        Write-Opt 11 "Remover Search Taskbar"           "Ocultar search box"
        Write-Opt 12 "Remover News Taskbar"             "Ocultar news and interests"
        Write-Opt 13 "Remover Cortana"                  "Desabilitar Cortana"
        Write-Opt 14 "Remover OneDrive Visual"          "Ocultar do File Explorer"
        Write-Opt 15 "Dark Mode"                        "Ativar tema escuro"
        ...
        switch (Read-Key) {
            ...
            "4"  { Run-Sub "tools/install_fdm" }
            "5"  { Run-Sub "tools/dcontrol" }
            "6"  { Run-Sub "tools/restore_point" }
            "7"  { Run-Sub "tweaks/set_homepage" }
            "8"  { Run-Sub "tools/list_j2534" }
            "9"  { Run-Sub "system/speedtest" }
            "10" { Write-Header; Write-Info "A lancar activador..."; irm https://get.activated.win | iex }
            "11" { Run-Sub "utils/remove_search_taskbar" }
            "12" { Run-Sub "utils/remove_news_taskbar" }
            "13" { Run-Sub "utils/remove_cortana" }
            "14" { Run-Sub "utils/remove_onedrive_visual" }
